...
public:
    ...
    //      or src column name == singular(table_name) + "Id"
    //   2. Value coverage: >= COVERAGE_THRESHOLD of src values exist in dst PK
    //
    // Handles both NUMERICAL and CATEGORICAL/TEXT PKs.
...
private:
    ...
    // NUMERICAL PK coverage: fraction of int64 values found in PK set
    static double compute_coverage_numerical(
        const Column& src_col,
        const Column& dst_pk_col
    );

    // STRING PK coverage: fraction of string values found in PK set
    static double compute_coverage_string(
        const Column& src_col,
        const Column& dst_pk_col
    );
};
